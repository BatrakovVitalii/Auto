...
class Player(Character):
    ...
    def use_portions(self):
        if self.portions > 0:
            self.health += 20
            self.portions -= 1
            print(f'{self.name} использует зелье и восстанавливает 20 здоровья')
        else:
            print('Нет зелий')
...
def print_turn(func):  # Печать начала и конца хода
...
def random_damage(func):  # Создание случайного урона
    def wrapper(*args, **kwargs):
...
def check_alive(func):  # Проверка живой ли
    def wrapper(*args, **kwargs):
        if args[0].is_alive():
            return func(*args, **kwargs)
        else:
